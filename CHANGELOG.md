...
- **kling-video-ai-avatar-v2-pro**: Added missing `audio` param_map — audio input was unmapped, causing provider requests to silently drop the audio file.
...
- **--force flag for fal-skill-scraper**: Scraper now supports `--force` to overwrite existing skill files.

### Changed

- **Directory restructure**: Migrated agents, skills, and docs into `kiani/` directory structure for better organization.
