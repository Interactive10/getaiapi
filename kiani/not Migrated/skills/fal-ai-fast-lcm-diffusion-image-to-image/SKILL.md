---
name: fal-ai-fast-lcm-diffusion-image-to-image
description: >
  Use this skill for the fal.ai Latent Consistency Models (v1.5/XL) model (fal-ai/fast-lcm-diffusion/image-to-image). Run SDXL at the speed of light
---
...
**Endpoint:** `fal-ai/fast-lcm-diffusion/image-to-image`
**Source:** https://fal.ai/models/fal-ai/fast-lcm-diffusion/image-to-image/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/fast-lcm-diffusion/image-to-image", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/fast-lcm-diffusion/image-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/fast-lcm-diffusion/image-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/fast-lcm-diffusion/image-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/fast-lcm-diffusion/image-to-image
