...
description: >
  Use this skill for the fal.ai FLUX.1 [dev] model (fal-ai/flux-1/dev). FLUX.1 [dev] is a 12 billion parameter flow transformer that generates high-quality images from text. It is suitable for personal and commercial use.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-1/dev/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-1/dev", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-1/dev", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-1/dev", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-1/dev/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-1/dev
