---
name: fal-ai-flux-2-flex-edit
description: >
  Use this skill for the fal.ai Flux 2 Flex model (fal-ai/flux-2-flex/edit). Image editing with FLUX.2 [flex] from Black Forest Labs. Supports multi-reference editing with customizable inference steps and enhanced text rendering.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-2-flex/edit/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2-flex/edit", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2-flex/edit", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2-flex/edit", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2-flex/edit/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2-flex/edit
