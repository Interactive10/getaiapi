---
name: fal-ai-flux-2-klein-4b
description: >
  Use this skill for the fal.ai Flux 2 [klein] 4B model (fal-ai/flux-2/klein/4b). Text-to-image generation with Flux 2 [klein] 4B from Black Forest Labs. Enhanced realism, crisper text generation, and native editing capabilities.
---
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2/klein/4b", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2/klein/4b", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2/klein/4b", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2/klein/4b/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2/klein/4b
