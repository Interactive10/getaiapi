---
name: fal-ai-flux-2-lora-gallery-hdr-style
description: >
  Use this skill for the fal.ai Flux 2 Lora Gallery model (fal-ai/flux-2-lora-gallery/hdr-style). HDR surrealistic effect with intense colors
---
...
**Source:** https://fal.ai/models/fal-ai/flux-2-lora-gallery/hdr-style/api
...
const result = await fal.subscribe("fal-ai/flux-2-lora-gallery/hdr-style", {
  input: {
...
| --- | --- | --- | --- | --- |
| `prompt` | string | **Yes** |  | The prompt to generate an HDR style image. The trigger word 'Hyp3rRe4list1c' will be automatically prepended. |
...
| `lora_scale` | float | No | `1` | The strength of the HDR style effect. |
...
| `images` | list<Image> | The generated HDR style images |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2-lora-gallery/hdr-style", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2-lora-gallery/hdr-style", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2-lora-gallery/hdr-style", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2-lora-gallery/hdr-style/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2-lora-gallery/hdr-style
