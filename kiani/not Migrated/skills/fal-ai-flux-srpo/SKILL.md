...
description: >
  Use this skill for the fal.ai FLUX.1 SRPO [dev] model (fal-ai/flux/srpo). FLUX.1 SRPO [dev] is a 12 billion parameter flow transformer that generates high-quality images from text with incredible aesthetics. It is suitable for personal and commercial use.
---
...
**Source:** https://fal.ai/models/fal-ai/flux/srpo/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux/srpo", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux/srpo", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux/srpo", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux/srpo/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux/srpo
