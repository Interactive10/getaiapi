---
name: fal-ai-fooocus-image-prompt
description: >
  Use this skill for the fal.ai Fooocus Image Prompt model (fal-ai/fooocus/image-prompt). Default parameters with automated optimizations and quality improvements.
---
...
**Endpoint:** `fal-ai/fooocus/image-prompt`
**Source:** https://fal.ai/models/fal-ai/fooocus/image-prompt/api
...
| `uov_image_url` | string | null | No |  | The image to upscale or vary. |
...
| `uov_method` | enum: `Disabled`, `Vary (Subtle)`, `Vary (Strong)`, `Upscale (1.5x)`, `Upscale (2x)`, `Upscale (Fast 2x)` | No | `"Disabled"` | The method to use for upscaling or varying. |
...
| `image_prompt_1` | ImagePrompt | **Yes** |  |  |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/fooocus/image-prompt", {
  input: {
        "image_prompt_1": "..."
    },
...
```javascript
const status = await fal.queue.status("fal-ai/fooocus/image-prompt", {
...
```javascript
const result = await fal.queue.result("fal-ai/fooocus/image-prompt", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/fooocus/image-prompt/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/fooocus/image-prompt
