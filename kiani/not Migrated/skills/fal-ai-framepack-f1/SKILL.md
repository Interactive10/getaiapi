---
name: fal-ai-framepack-f1
description: >
  Use this skill for the fal.ai Framepack F1 model (fal-ai/framepack/f1). Framepack is an efficient Image-to-video model that autoregressively generates videos.
---
...
**Source:** https://fal.ai/models/fal-ai/framepack/f1/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/framepack/f1", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/framepack/f1", {
...
```javascript
const result = await fal.queue.result("fal-ai/framepack/f1", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/framepack/f1/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/framepack/f1
