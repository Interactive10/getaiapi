---
name: fal-ai-framepack-flf2v
description: >
  Use this skill for the fal.ai Framepack model (fal-ai/framepack/flf2v). Framepack is an efficient Image-to-video model that autoregressively generates videos.
---
...
**Endpoint:** `fal-ai/framepack/flf2v`
**Source:** https://fal.ai/models/fal-ai/framepack/flf2v/api
...
| `num_frames` | integer | No | `240` | The number of frames to generate. |
| `image_url` | string | **Yes** |  | URL of the image input. |
| `strength` | float | No | `0.8` | Determines the influence of the final frame on the generated video. Higher values result in the output being more hea... |
...
| `end_image_url` | string | **Yes** |  | URL of the end image input. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/framepack/flf2v", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png",
        "end_image_url": "https://example.com/input.png"
...
```javascript
const status = await fal.queue.status("fal-ai/framepack/flf2v", {
...
```javascript
const result = await fal.queue.result("fal-ai/framepack/flf2v", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/framepack/flf2v/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/framepack/flf2v
