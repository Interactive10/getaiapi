---
name: fal-ai-hunyuan-video-lora
description: >
  Use this skill for the fal.ai Hunyuan Video LoRA Inference model (fal-ai/hunyuan-video-lora). Hunyuan Video is an Open video generation model with high visual quality, motion diversity, text-video alignment, and generation stability
---
...
**Endpoint:** `fal-ai/hunyuan-video-lora`
**Source:** https://fal.ai/models/fal-ai/hunyuan-video-lora/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/hunyuan-video-lora", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/hunyuan-video-lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/hunyuan-video-lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/hunyuan-video-lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/hunyuan-video-lora
