---
name: fal-ai-ideogram-v3-remix
description: >
  Use this skill for the fal.ai Ideogram model (fal-ai/ideogram/v3/remix). Reimagine existing images with Ideogram V3's remix feature. Create variations and adaptations while preserving core elements and adding new creative directions through prompt guidance.
---
...
**Endpoint:** `fal-ai/ideogram/v3/remix`
**Source:** https://fal.ai/models/fal-ai/ideogram/v3/remix/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ideogram/v3/remix", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ideogram/v3/remix", {
...
```javascript
const result = await fal.queue.result("fal-ai/ideogram/v3/remix", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ideogram/v3/remix/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ideogram/v3/remix
