...
name: fal-ai-imagen3
description: >
  Use this skill for the fal.ai Imagen3 model (fal-ai/imagen3). Imagen3 is a high-quality text-to-image model that generates realistic images from text prompts.
---
...
**Source:** https://fal.ai/models/fal-ai/imagen3/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/imagen3", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/imagen3", {
...
```javascript
const result = await fal.queue.result("fal-ai/imagen3", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/imagen3/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/imagen3
