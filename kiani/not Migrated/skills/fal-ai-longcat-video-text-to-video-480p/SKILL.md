...
description: >
  Use this skill for the fal.ai LongCat Video model (fal-ai/longcat-video/text-to-video/480p). Generate long videos from text using LongCat Video
---
...
**Endpoint:** `fal-ai/longcat-video/text-to-video/480p`
**Source:** https://fal.ai/models/fal-ai/longcat-video/text-to-video/480p/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/longcat-video/text-to-video/480p", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/longcat-video/text-to-video/480p", {
...
```javascript
const result = await fal.queue.result("fal-ai/longcat-video/text-to-video/480p", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/longcat-video/text-to-video/480p/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/longcat-video/text-to-video/480p
