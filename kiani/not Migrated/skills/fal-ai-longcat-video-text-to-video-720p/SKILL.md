...
description: >
  Use this skill for the fal.ai LongCat Video model (fal-ai/longcat-video/text-to-video/720p). Generate long videos in 720p/30fps from text using LongCat Video
---
...
**Source:** https://fal.ai/models/fal-ai/longcat-video/text-to-video/720p/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/longcat-video/text-to-video/720p", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/longcat-video/text-to-video/720p", {
...
```javascript
const result = await fal.queue.result("fal-ai/longcat-video/text-to-video/720p", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/longcat-video/text-to-video/720p/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/longcat-video/text-to-video/720p
