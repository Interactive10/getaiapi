---
name: fal-ai-ltx-2-19b-distilled-extend-video-lora
description: >
  Use this skill for the fal.ai LTX-2 19B Distilled model (fal-ai/ltx-2-19b/distilled/extend-video/lora). Extend videos with audio using LTX-2 Distilled and custom LoRA
---
...
**Endpoint:** `fal-ai/ltx-2-19b/distilled/extend-video/lora`
**Source:** https://fal.ai/models/fal-ai/ltx-2-19b/distilled/extend-video/lora/api
...
const result = await fal.subscribe("fal-ai/ltx-2-19b/distilled/extend-video/lora", {
  input: {
...
| `extend_direction` | enum: `forward`, `backward` | No | `"forward"` | Direction to extend the video. 'forward' extends from the end of the video, 'backward' extends from the beginning. |
...
| `end_image_url` | string | null | No |  | The URL of the image to use as the end of the extended video. |
...
| `num_context_frames` | integer | No | `25` | The number of frames to use as context for the extension. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-2-19b/distilled/extend-video/lora", {
  input: {
        "video_url": "https://example.com/input.png",
        "prompt": "your value here",
        "loras": []
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-2-19b/distilled/extend-video/lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-2-19b/distilled/extend-video/lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-2-19b/distilled/extend-video/lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2-19b/distilled/extend-video/lora
