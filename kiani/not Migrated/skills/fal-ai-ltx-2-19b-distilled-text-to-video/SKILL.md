---
name: fal-ai-ltx-2-19b-distilled-text-to-video
description: >
  Use this skill for the fal.ai LTX-2 19B Distilled model (fal-ai/ltx-2-19b/distilled/text-to-video). Generate video with audio from text using LTX-2 Distilled
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-2-19b/distilled/text-to-video/api
...
const result = await fal.subscribe("fal-ai/ltx-2-19b/distilled/text-to-video", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-2-19b/distilled/text-to-video", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-2-19b/distilled/text-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-2-19b/distilled/text-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-2-19b/distilled/text-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2-19b/distilled/text-to-video
