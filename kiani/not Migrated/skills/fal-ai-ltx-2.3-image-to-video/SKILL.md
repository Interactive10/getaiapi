...
description: >
  Use this skill for the fal.ai LTX 2.3 Video Pro model (fal-ai/ltx-2.3/image-to-video). LTX-2.3 is a high-quality, fast AI video model available in Pro and Fast variants for text-to-video, image-to-video, and audio-to-video.
---
...
| `aspect_ratio` | enum: `auto`, `16:9`, `9:16` | No | `"auto"` | The aspect ratio of the generated video. If 'auto', the aspect ratio will be determined automatically based on the in... |
| `image_url` | string | **Yes** |  | The URL of the start image to use for the generated video. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-2.3/image-to-video", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-2.3/image-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-2.3/image-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-2.3/image-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2.3/image-to-video
