...
description: >
  Use this skill for the fal.ai LTX 2.3 Video Fast model (fal-ai/ltx-2.3/text-to-video/fast). LTX-2.3 is a high-quality, fast AI video model available in Pro and Fast variants for text-to-video, image-to-video, and audio-to-video.
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-2.3/text-to-video/fast/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-2.3/text-to-video/fast", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-2.3/text-to-video/fast", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-2.3/text-to-video/fast", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-2.3/text-to-video/fast/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2.3/text-to-video/fast
