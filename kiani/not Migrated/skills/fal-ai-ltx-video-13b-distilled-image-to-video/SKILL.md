---
name: fal-ai-ltx-video-13b-distilled-image-to-video
description: >
  Use this skill for the fal.ai LTX Video-0.9.7 13B Distilled model (fal-ai/ltx-video-13b-distilled/image-to-video). Generate videos from prompts and images using LTX Video-0.9.7 13B Distilled and custom LoRA
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-video-13b-distilled/image-to-video/api
...
const result = await fal.subscribe("fal-ai/ltx-video-13b-distilled/image-to-video", {
  input: {
...
| `image_url` | string | **Yes** |  | Image URL for Image-to-Video task |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-video-13b-distilled/image-to-video", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-video-13b-distilled/image-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-video-13b-distilled/image-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-video-13b-distilled/image-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-video-13b-distilled/image-to-video
