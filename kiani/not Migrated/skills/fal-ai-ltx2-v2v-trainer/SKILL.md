---
name: fal-ai-ltx2-v2v-trainer
description: >
  Use this skill for the fal.ai LTX-2 Video to Video Trainer model (fal-ai/ltx2-v2v-trainer). Train LTX-2 for video transformation or video-conditioned generation.
---

# LTX-2 Video to Video Trainer

Train LTX-2 for video transformation or video-conditioned generation.

**Endpoint:** `fal-ai/ltx2-v2v-trainer`
**Source:** https://fal.ai/models/fal-ai/ltx2-v2v-trainer/api
...
const result = await fal.subscribe("fal-ai/ltx2-v2v-trainer", {
  input: {
...
| `validation` | list<V2VValidation> | No | `[]` | A list of validation inputs with prompts and reference videos. |
...
| `first_frame_conditioning_p` | float | No | `0.1` | Probability of conditioning on the first frame during training. Lower values work better for video-to-video transform... |
...
| `lora_file` | File |  |
| `config_file` | File |  |
| `debug_dataset` | File | null | URL to the debug dataset archive containing decoded videos. |
| `video` | File | null | The URL to the validation videos (with reference videos side-by-side), if any. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx2-v2v-trainer", {
  input: {
        "training_data_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx2-v2v-trainer", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx2-v2v-trainer", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx2-v2v-trainer/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx2-v2v-trainer
