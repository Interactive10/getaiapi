...
description: >
  Use this skill for the fal.ai MiniMax Speech 2.6 [HD] model (fal-ai/minimax/speech-2.6-hd). Generate speech from text prompts and different voices using the MiniMax Speech-2.6 HD model, which leverages advanced AI techniques to create high-quality text-to-speech.
---
...
**Source:** https://fal.ai/models/fal-ai/minimax/speech-2.6-hd/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/minimax/speech-2.6-hd", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/minimax/speech-2.6-hd", {
...
```javascript
const result = await fal.queue.result("fal-ai/minimax/speech-2.6-hd", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/minimax/speech-2.6-hd/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/minimax/speech-2.6-hd
