...
description: >
  Use this skill for the fal.ai Qwen 3 TTS - Text to Speech [1.7B] model (fal-ai/qwen-3-tts/text-to-speech/1.7b). Bring speech to your texts using Qwen3-TTS Custom-Voice model with pre-trained voices or use your custom voice with Qwen3-TTS Clone Voice model
---
...
**Source:** https://fal.ai/models/fal-ai/qwen-3-tts/text-to-speech/1.7b/api
...
| `speaker_voice_embedding_file_url` | string | null | No |  | URL to a speaker embedding file in safetensors format, from `fal-ai/qwen-3-tts/clone-voice` endpoint. If provided, th... |
...
```javascript
const status = await fal.queue.status("fal-ai/qwen-3-tts/text-to-speech/1.7b", {
...
```javascript
const result = await fal.queue.result("fal-ai/qwen-3-tts/text-to-speech/1.7b", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/qwen-3-tts/text-to-speech/1.7b/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/qwen-3-tts/text-to-speech/1.7b
