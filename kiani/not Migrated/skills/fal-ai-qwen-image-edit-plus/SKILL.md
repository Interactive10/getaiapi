---
name: fal-ai-qwen-image-edit-plus
description: >
  Use this skill for the fal.ai Qwen Image Edit Plus model (fal-ai/qwen-image-edit-plus). Endpoint for Qwen's Image Editing Plus model also known as Qwen-Image-Edit-2509. Has superior text editing capabilities and multi-image support.
---
...
**Source:** https://fal.ai/models/fal-ai/qwen-image-edit-plus/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/qwen-image-edit-plus", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/qwen-image-edit-plus", {
...
```javascript
const result = await fal.queue.result("fal-ai/qwen-image-edit-plus", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/qwen-image-edit-plus/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/qwen-image-edit-plus
