---
name: fal-ai-sam-audio-visual-separate
description: >
  Use this skill for the fal.ai Sam Audio model (fal-ai/sam-audio/visual-separate). Audio separation with SAM Audio. Isolate any sound using natural language—professional-grade audio editing made simple for creators, researchers, and accessibility applications.
---
...
**Endpoint:** `fal-ai/sam-audio/visual-separate`
**Source:** https://fal.ai/models/fal-ai/sam-audio/visual-separate/api
...
| --- | --- | --- | --- | --- |
| `prompt` | string | No | `""` | Text prompt to assist with separation. Use natural language to describe the target sound. |
| `video_url` | string | **Yes** |  | URL of the video file to process (MP4, MOV, etc.) |
...
| `mask_video_url` | string | null | No |  | URL of the mask video (binary mask indicating target object). Black=target, White=background. |
...
- API page: https://fal.ai/models/fal-ai/sam-audio/visual-separate/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/sam-audio/visual-separate
