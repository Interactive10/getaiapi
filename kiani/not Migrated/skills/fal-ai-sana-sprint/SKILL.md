---
name: fal-ai-sana-sprint
description: >
  Use this skill for the fal.ai Sana Sprint model (fal-ai/sana/sprint). Sana Sprint is a text-to-image model capable of generating 4K images with exceptional speed.
---
...
**Source:** https://fal.ai/models/fal-ai/sana/sprint/api
...
const result = await fal.subscribe("fal-ai/sana/sprint", {
  input: {
...
| `num_inference_steps` | integer | No | `2` | The number of inference steps to perform. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/sana/sprint", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/sana/sprint", {
...
```javascript
const result = await fal.queue.result("fal-ai/sana/sprint", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/sana/sprint/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/sana/sprint
