...
description: >
  Use this skill for the fal.ai Stable Diffusion 3.5 Medium model (fal-ai/stable-diffusion-v35-medium). Stable Diffusion 3.5 Medium is a Multimodal Diffusion Transformer (MMDiT) text-to-image model that features improved performance in image quality, typography, complex prompt understanding, and resourc
---
...
**Source:** https://fal.ai/models/fal-ai/stable-diffusion-v35-medium/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/stable-diffusion-v35-medium", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/stable-diffusion-v35-medium", {
...
```javascript
const result = await fal.queue.result("fal-ai/stable-diffusion-v35-medium", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/stable-diffusion-v35-medium/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/stable-diffusion-v35-medium
