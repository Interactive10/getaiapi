---
name: fal-ai-switti-512
description: >
  Use this skill for the fal.ai Switti 512 model (fal-ai/switti/512). Switti is a scale-wise transformer for fast text-to-image generation that outperforms existing T2I AR models and competes with state-of-the-art T2I diffusion models while being faster than distilled d
---
...
**Endpoint:** `fal-ai/switti/512`
**Source:** https://fal.ai/models/fal-ai/switti/512/api
...
const result = await fal.subscribe("fal-ai/switti/512", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/switti/512", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/switti/512", {
...
```javascript
const result = await fal.queue.result("fal-ai/switti/512", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/switti/512/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/switti/512
