---
name: fal-ai-thinksound
description: >
  Use this skill for the fal.ai ThinkSound model (fal-ai/thinksound). Generate realistic audio for a video with an optional text prompt and combine
---
...
**Source:** https://fal.ai/models/fal-ai/thinksound/api
...
| `video` | File | The generated video with audio. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/thinksound", {
  input: {
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/thinksound", {
...
```javascript
const result = await fal.queue.result("fal-ai/thinksound", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/thinksound/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/thinksound
