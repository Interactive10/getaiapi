---
name: fal-ai-wan-trainer
description: >
  Use this skill for the fal.ai Wan-2.1 LoRA Trainer model (fal-ai/wan-trainer). Train custom LoRAs for Wan-2.1 I2V 480P
---
...
**Source:** https://fal.ai/models/fal-ai/wan-trainer/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-trainer", {
  input: {
        "training_data_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-trainer", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-trainer", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-trainer/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-trainer
