...
description: >
  Use this skill for the fal.ai Wan model (fal-ai/wan/v2.2-a14b/text-to-image). Wan 2.2's 14B model generates high-resolution, photorealistic images with powerful prompt understanding and fine-grained visual detail
---
...
**Endpoint:** `fal-ai/wan/v2.2-a14b/text-to-image`
**Source:** https://fal.ai/models/fal-ai/wan/v2.2-a14b/text-to-image/api
...
const result = await fal.subscribe("fal-ai/wan/v2.2-a14b/text-to-image", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan/v2.2-a14b/text-to-image", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan/v2.2-a14b/text-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan/v2.2-a14b/text-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan/v2.2-a14b/text-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan/v2.2-a14b/text-to-image
