---
name: fal-ai-wan-v2.2-a14b-text-to-video-lora
description: >
  Use this skill for the fal.ai Wan-2.2 Text-to-Video A14B with LoRAs model (fal-ai/wan/v2.2-a14b/text-to-video/lora). Wan-2.2 text-to-video is a video model that generates high-quality videos with high visual quality and motion diversity from text prompts. This endpoint supports LoRAs made for Wan 2.2.
---
...
**Source:** https://fal.ai/models/fal-ai/wan/v2.2-a14b/text-to-video/lora/api
...
const result = await fal.subscribe("fal-ai/wan/v2.2-a14b/text-to-video/lora", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan/v2.2-a14b/text-to-video/lora", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan/v2.2-a14b/text-to-video/lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan/v2.2-a14b/text-to-video/lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan/v2.2-a14b/text-to-video/lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan/v2.2-a14b/text-to-video/lora
