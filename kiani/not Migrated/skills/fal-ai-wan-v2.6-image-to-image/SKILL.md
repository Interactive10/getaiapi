---
name: fal-ai-wan-v2.6-image-to-image
description: >
  Use this skill for the fal.ai Wan v2.6 Image to Image model (wan/v2.6/image-to-image). Wan 2.6 image-to-image model.
---
...
**Source:** https://fal.ai/models/wan/v2.6/image-to-image/api
...
| `image_size` | ImageSize | enum: `square_hd`, `square`, `portrait_4_3`, `portrait_16_9`, `landscape_4_3`, `landscape_16_9` | No | `"square_hd"` | Output image size. Use presets like 'square_hd', 'landscape_16_9', 'portrait_9_16', or specify exact dimensions with ... |
...
| `seed` | integer | null | No |  | Random seed for reproducibility (0-2147483647). Same seed produces more consistent results. |
...
```javascript
const { request_id } = await fal.queue.submit("wan/v2.6/image-to-image", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("wan/v2.6/image-to-image", {
...
```javascript
const result = await fal.queue.result("wan/v2.6/image-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/wan/v2.6/image-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=wan/v2.6/image-to-image
