---
name: fal-ai-wan-vace-1-3b
description: >
  Use this skill for the fal.ai Wan Vace 1 3b model (fal-ai/wan-vace-1-3b). Vace a video generation model that uses a source image, mask, and video to create prompted videos with controllable sources.
---
...
**Endpoint:** `fal-ai/wan-vace-1-3b`
**Source:** https://fal.ai/models/fal-ai/wan-vace-1-3b/api
...
const result = await fal.subscribe("fal-ai/wan-vace-1-3b", {
  input: {
...
| `task` | enum: `depth`, `inpainting`, `pose` | No | `"depth"` | Task type for the model. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-vace-1-3b", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-vace-1-3b", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-vace-1-3b", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-vace-1-3b/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-vace-1-3b
