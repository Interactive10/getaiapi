---
name: fal-ai-wan-vace-14b-outpainting
description: >
  Use this skill for the fal.ai Wan VACE 14B model (fal-ai/wan-vace-14b/outpainting). VACE is a video generation model that uses a source image, mask, and video to create prompted videos with controllable sources.
---
...
**Source:** https://fal.ai/models/fal-ai/wan-vace-14b/outpainting/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-vace-14b/outpainting", {
  input: {
        "prompt": "your value here",
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-vace-14b/outpainting", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-vace-14b/outpainting", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-vace-14b/outpainting/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-vace-14b/outpainting
