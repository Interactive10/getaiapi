---
name: fal-ai-z-image-base-lora
description: >
  Use this skill for the fal.ai Z Image Base (LoRA) model (fal-ai/z-image/base/lora). LoRA endpoint for Z-Image, the foundation model of the Z- Image family.
---
...
**Endpoint:** `fal-ai/z-image/base/lora`
**Source:** https://fal.ai/models/fal-ai/z-image/base/lora/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/z-image/base/lora", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/z-image/base/lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/z-image/base/lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/z-image/base/lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/z-image/base/lora
