---
name: fal-ai-z-image-turbo-image-to-image-lora
description: >
  Use this skill for the fal.ai Z-Image Turbo model (fal-ai/z-image/turbo/image-to-image/lora). Generate images from text and images using custom LoRA and Z-Image Turbo, Tongyi-MAI's super-fast 6B model.
---
...
**Source:** https://fal.ai/models/fal-ai/z-image/turbo/image-to-image/lora/api
...
| `strength` | float | No | `0.6` | The strength of the image-to-image conditioning. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/z-image/turbo/image-to-image/lora", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/z-image/turbo/image-to-image/lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/z-image/turbo/image-to-image/lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/z-image/turbo/image-to-image/lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/z-image/turbo/image-to-image/lora
