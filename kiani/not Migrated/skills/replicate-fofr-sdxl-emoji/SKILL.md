---
name: replicate-fofr-sdxl-emoji
description: >
  Use this skill for the Replicate Sdxl Emoji model (fofr/sdxl-emoji). Use the Sdxl Emoji model via Replicate API.
---
...
const output = await replicate.run("fofr/sdxl-emoji", {
  input: {
...
output = replicate.run("fofr/sdxl-emoji",
    input={
        "prompt": "your prompt here"
    },
)
...
```bash
curl -s -X POST https://api.replicate.com/v1/models/fofr/sdxl-emoji/predictions \
...
- Model page: https://replicate.com/fofr/sdxl-emoji
- API page: https://replicate.com/fofr/sdxl-emoji/api
