...
description: >
  Use this skill for the Kling AI Kling AI Multi-Shot API (/v1/images/ai-multi-shot).
  Generate consistent multi-shot image series using the Kling AI API. Create character-consistent image sequences for storyboards and comics.
---
...
**Create Task:** `POST /v1/images/ai-multi-shot`
**Query Task:** `GET /v1/images/ai-multi-shot/{task_id}`
**Source:** [Kling API Docs](https://kling.ai/document-api/apiReference%2Fmodel%2FaiMultiShot)
...
```bash
curl --request POST \
  --url https://api-singapore.klingai.com/v1/images/ai-multi-shot \
...
resp = requests.post(
    "https://api-singapore.klingai.com/v1/images/ai-multi-shot",
    json=payload,
    headers=HEADERS,
)
...
while True:
    result = requests.get(
        f"https://api-singapore.klingai.com/v1/images/ai-multi-shot/{task_id}",
        headers=HEADERS,
    ).json()
...
// 1. Create task
const createResp = await fetch("https://api-singapore.klingai.com/v1/images/ai-multi-shot", {
  method: "POST",
...
// 2. Poll for result
while (true) {
  const resp = await fetch(`https://api-singapore.klingai.com/v1/images/ai-multi-shot/${taskId}`, {
...
| --- | --- | --- | --- | --- |
| `element_frontal_image` | string | **Yes** | — | The frontal side of element image Supports inputting image Base64 encoding or image URL (ensure accessibility). Supported image formats: .jpg / .jpeg / .png Image file size cannot exceed 10MB, and the |
...
| `external_task_id` | string | No | — | Customized Task ID Users can provide a customized task ID, which will not overwrite the system-generated task ID but can be used for task queries Please note that the customized task ID must be unique |
...
1. **Create Task** — `POST /v1/images/ai-multi-shot` → returns `task_id` with status `submitted`
2. **Poll Status** — `GET /v1/images/ai-multi-shot/{task_id}` → status cycles: `submitted` → `processing` → `succeed` / `failed`
...
- [Kling API Documentation](https://kling.ai/document-api/apiReference%2Fmodel%2FaiMultiShot)
