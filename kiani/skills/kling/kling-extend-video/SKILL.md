...
description: >
  Use this skill for the Kling AI Kling Extend Video API (/v1/videos/video-extend).
  Extend existing videos using the Kling AI API. Continue a generated video by appending additional frames with optional prompt guidance.
---
...
**Query Task:** `GET /v1/videos/video-extend/{task_id}`
**Source:** [Kling API Docs](https://kling.ai/document-api/apiReference%2Fmodel%2FvideoExtension)
...
resp = requests.post(
    "https://api-singapore.klingai.com/v1/videos/video-extend",
    json=payload,
    headers=HEADERS,
)
...
while True:
    result = requests.get(
        f"https://api-singapore.klingai.com/v1/videos/video-extend/{task_id}",
        headers=HEADERS,
    ).json()
...
// 1. Create task
const createResp = await fetch("https://api-singapore.klingai.com/v1/videos/video-extend", {
  method: "POST",
...
// 2. Poll for result
while (true) {
  const resp = await fetch(`https://api-singapore.klingai.com/v1/videos/video-extend/${taskId}`, {
...
| --- | --- | --- | --- | --- |
| `video_id` | string | **Yes** | — | Video ID Supports video IDs generated by the text-to-video, the image-to-video and the video extension interface (it cannot exceed 3 minutes). Only videos generated by V1.0, V1.5, and V1.6 models are  |
| `prompt` | string | No | — | Text PromptCannot exceed 2500 characters |
| `negative_prompt` | string | No | — | Negative text promptCannot exceed 2500 characters |
...
1. **Create Task** — `POST /v1/videos/video-extend` → returns `task_id` with status `submitted`
2. **Poll Status** — `GET /v1/videos/video-extend/{task_id}` → status cycles: `submitted` → `processing` → `succeed` / `failed`
...
- [Kling API Documentation](https://kling.ai/document-api/apiReference%2Fmodel%2FvideoExtension)
