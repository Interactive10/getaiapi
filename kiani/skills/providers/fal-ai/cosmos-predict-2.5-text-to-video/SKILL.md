---
name: fal-ai-cosmos-predict-2.5-text-to-video
description: >
  Use this skill for the fal.ai Cosmos Predict 2.5 2B model (fal-ai/cosmos-predict-2.5/text-to-video). Generate video from text using NVIDIA's 2B Cosmos Post-Trained Model
---
...
**Endpoint:** `fal-ai/cosmos-predict-2.5/text-to-video`
**Source:** https://fal.ai/models/fal-ai/cosmos-predict-2.5/text-to-video/api
...
const result = await fal.subscribe("fal-ai/cosmos-predict-2.5/text-to-video", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/cosmos-predict-2.5/text-to-video", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/cosmos-predict-2.5/text-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/cosmos-predict-2.5/text-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/cosmos-predict-2.5/text-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/cosmos-predict-2.5/text-to-video
