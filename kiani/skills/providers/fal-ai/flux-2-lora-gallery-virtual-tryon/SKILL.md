---
name: fal-ai-flux-2-lora-gallery-virtual-tryon
description: >
  Use this skill for the fal.ai Flux 2 Lora Gallery model (fal-ai/flux-2-lora-gallery/virtual-tryon). Virtual clothing try-on (2 images: person + garment)
---
...
**Source:** https://fal.ai/models/fal-ai/flux-2-lora-gallery/virtual-tryon/api
...
| --- | --- | --- | --- | --- |
| `prompt` | string | **Yes** |  | The prompt to generate a virtual try-on image. |
...
| `lora_scale` | float | No | `1` | The strength of the virtual try-on effect. |
...
| `image_urls` | list<string> | **Yes** |  | The URLs of the images for virtual try-on. Provide person image and clothing image. |
...
| `images` | list<Image> | The generated virtual try-on images |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2-lora-gallery/virtual-tryon", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2-lora-gallery/virtual-tryon", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2-lora-gallery/virtual-tryon", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2-lora-gallery/virtual-tryon/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2-lora-gallery/virtual-tryon
