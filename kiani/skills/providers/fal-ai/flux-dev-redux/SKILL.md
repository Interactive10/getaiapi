---
name: fal-ai-flux-dev-redux
description: >
  Use this skill for the fal.ai FLUX.1 [dev] Redux model (fal-ai/flux/dev/redux). FLUX.1 [dev] Redux is a high-performance endpoint for the FLUX.1 [dev] model that enables rapid transformation of existing images, delivering high-quality style transfers and image modifications with 
---
...
**Source:** https://fal.ai/models/fal-ai/flux/dev/redux/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux/dev/redux", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux/dev/redux", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux/dev/redux", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux/dev/redux/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux/dev/redux
