---
name: fal-ai-flux-lora-stream
description: >
  Use this skill for the fal.ai Flux Lora model (fal-ai/flux-lora/stream). Super fast endpoint for the FLUX.1 [dev] model with LoRA support, enabling rapid and high-quality image generation using pre-trained LoRA adaptations for personalization, specific styles, brand identi
---
...
**Endpoint:** `fal-ai/flux-lora/stream`
**Source:** https://fal.ai/models/fal-ai/flux-lora/stream/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-lora/stream", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-lora/stream", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-lora/stream", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-lora/stream/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-lora/stream
