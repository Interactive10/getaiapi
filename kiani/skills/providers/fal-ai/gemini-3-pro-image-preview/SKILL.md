...
description: >
  Use this skill for the fal.ai Gemini 3 Pro Image Preview model (fal-ai/gemini-3-pro-image-preview). Gemini 3 Pro Image (a.k.a Nano Banana Pro) is Google's state-of-the-art high-fidelity image generation and editing model
---
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/gemini-3-pro-image-preview", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/gemini-3-pro-image-preview", {
...
```javascript
const result = await fal.queue.result("fal-ai/gemini-3-pro-image-preview", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/gemini-3-pro-image-preview/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/gemini-3-pro-image-preview
