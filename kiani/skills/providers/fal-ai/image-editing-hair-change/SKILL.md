---
name: fal-ai-image-editing-hair-change
description: >
  Use this skill for the fal.ai Image Editing model (fal-ai/image-editing/hair-change). Experiment with different hairstyles, from bald to any style you can imagine, while maintaining natural lighting and realistic results.
---
...
**Source:** https://fal.ai/models/fal-ai/image-editing/hair-change/api
...
| --- | --- | --- | --- | --- |
| `prompt` | string | No | `"bald"` | The desired hair style to apply. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/image-editing/hair-change", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/image-editing/hair-change", {
...
```javascript
const result = await fal.queue.result("fal-ai/image-editing/hair-change", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/image-editing/hair-change/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/image-editing/hair-change
