---
name: fal-ai-longcat-image
description: >
  Use this skill for the fal.ai Longcat Image model (fal-ai/longcat-image). LongCat image is a 6B parameter model excelling at multilingual text rendering, photorealism and deployment efficiency.
---
...
**Source:** https://fal.ai/models/fal-ai/longcat-image/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/longcat-image", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/longcat-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/longcat-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/longcat-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/longcat-image
