---
name: fal-ai-longcat-video-distilled-image-to-video-720p
description: >
  Use this skill for the fal.ai LongCat Video Distilled model (fal-ai/longcat-video/distilled/image-to-video/720p). Generate long videos in 720p/30fps from images using LongCat Video Distilled
---
...
**Source:** https://fal.ai/models/fal-ai/longcat-video/distilled/image-to-video/720p/api
...
const result = await fal.subscribe("fal-ai/longcat-video/distilled/image-to-video/720p", {
  input: {
...
| `fps` | integer | No | `30` | The frame rate of the generated video. |
| `sync_mode` | boolean | No | `false` | If `True`, the media will be returned as a data URI and the output data won't be available in the request history. |
| `num_refine_inference_steps` | integer | No | `12` | The number of inference steps to use for refinement. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/longcat-video/distilled/image-to-video/720p", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/longcat-video/distilled/image-to-video/720p", {
...
```javascript
const result = await fal.queue.result("fal-ai/longcat-video/distilled/image-to-video/720p", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/longcat-video/distilled/image-to-video/720p/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/longcat-video/distilled/image-to-video/720p
