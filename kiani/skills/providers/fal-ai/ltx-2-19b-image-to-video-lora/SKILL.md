---
name: fal-ai-ltx-2-19b-image-to-video-lora
description: >
  Use this skill for the fal.ai LTX-2 19B model (fal-ai/ltx-2-19b/image-to-video/lora). Generate video with audio from images using LTX-2 and custom LoRA
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-2-19b/image-to-video/lora/api
...
const result = await fal.subscribe("fal-ai/ltx-2-19b/image-to-video/lora", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-2-19b/image-to-video/lora", {
  input: {
        "prompt": "your value here",
        "loras": [],
        "image_url": "https://example.com/input.png"
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-2-19b/image-to-video/lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-2-19b/image-to-video/lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-2-19b/image-to-video/lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2-19b/image-to-video/lora
