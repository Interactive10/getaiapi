---
name: fal-ai-ltx-video-13b-distilled-extend
description: >
  Use this skill for the fal.ai LTX Video-0.9.7 13B Distilled model (fal-ai/ltx-video-13b-distilled/extend). Extend videos using LTX Video-0.9.7 13B Distilled and custom LoRA
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-video-13b-distilled/extend/api
...
const result = await fal.subscribe("fal-ai/ltx-video-13b-distilled/extend", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-video-13b-distilled/extend", {
  input: {
        "prompt": "your value here",
        "video": "..."
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-video-13b-distilled/extend", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-video-13b-distilled/extend", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-video-13b-distilled/extend/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-video-13b-distilled/extend
