---
name: fal-ai-ltx-video-13b-distilled
description: >
  Use this skill for the fal.ai LTX Video-0.9.7 13B Distilled model (fal-ai/ltx-video-13b-distilled). Generate videos from prompts using LTX Video-0.9.7 13B Distilled and custom LoRA
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-video-13b-distilled/api
...
| `aspect_ratio` | enum: `9:16`, `1:1`, `16:9` | No | `"16:9"` | Aspect ratio of the generated video. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-video-13b-distilled", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-video-13b-distilled", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-video-13b-distilled", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-video-13b-distilled/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-video-13b-distilled
