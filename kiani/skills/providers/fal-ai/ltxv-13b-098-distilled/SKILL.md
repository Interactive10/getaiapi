---
name: fal-ai-ltxv-13b-098-distilled
description: >
  Use this skill for the fal.ai LTX-Video 13B 0.9.8 Distilled model (fal-ai/ltxv-13b-098-distilled). Generate long videos from prompts using LTX Video-0.9.8 13B Distilled and custom LoRA
---
...
**Source:** https://fal.ai/models/fal-ai/ltxv-13b-098-distilled/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltxv-13b-098-distilled", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltxv-13b-098-distilled", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltxv-13b-098-distilled", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltxv-13b-098-distilled/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltxv-13b-098-distilled
