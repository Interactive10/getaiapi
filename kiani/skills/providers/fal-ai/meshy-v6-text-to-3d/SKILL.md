...
description: >
  Use this skill for the fal.ai Meshy 6 model (fal-ai/meshy/v6/text-to-3d). Meshy-6 is the latest model from Meshy. It generates realistic and production ready 3D models.
---
...
**Source:** https://fal.ai/models/fal-ai/meshy/v6/text-to-3d/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/meshy/v6/text-to-3d", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/meshy/v6/text-to-3d", {
...
```javascript
const result = await fal.queue.result("fal-ai/meshy/v6/text-to-3d", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/meshy/v6/text-to-3d/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/meshy/v6/text-to-3d
