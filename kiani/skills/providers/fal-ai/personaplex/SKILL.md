---
name: fal-ai-personaplex
description: >
  Use this skill for the fal.ai Personaplex model (fal-ai/personaplex). PersonaPlex is a real-time, full-duplex speech-to-speech conversational model that enables persona control through text-based role prompts and audio-based voice conditioning.
---
...
**Endpoint:** `fal-ai/personaplex`
**Source:** https://fal.ai/models/fal-ai/personaplex/api
...
| `audio_url` | string | **Yes** |  | URL to the input audio file (user's speech). |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/personaplex", {
  input: {
        "audio_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/personaplex", {
...
```javascript
const result = await fal.queue.result("fal-ai/personaplex", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/personaplex/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/personaplex
