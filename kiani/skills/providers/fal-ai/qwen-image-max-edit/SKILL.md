---
name: fal-ai-qwen-image-max-edit
description: >
  Use this skill for the fal.ai Qwen Image Max model (fal-ai/qwen-image-max/edit). Image editing endpoint for Qwen-Image-Max. Qwen Image Max improves upon the Qwen Image Plus series by enhancing the realism and naturalness of images.
---
...
**Source:** https://fal.ai/models/fal-ai/qwen-image-max/edit/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/qwen-image-max/edit", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/qwen-image-max/edit", {
...
```javascript
const result = await fal.queue.result("fal-ai/qwen-image-max/edit", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/qwen-image-max/edit/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/qwen-image-max/edit
