---
name: fal-ai-qwen-image
description: >
  Use this skill for the fal.ai Qwen Image model (fal-ai/qwen-image). Qwen-Image is an image generation foundation model in the Qwen series that achieves significant advances in complex text rendering and precise image editing.
---
...
**Source:** https://fal.ai/models/fal-ai/qwen-image/api
...
```javascript
const status = await fal.queue.status("fal-ai/qwen-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/qwen-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/qwen-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/qwen-image
