---
name: fal-ai-sam-3-image-rle
description: >
  Use this skill for the fal.ai Sam 3 model (fal-ai/sam-3/image-rle). SAM 3 is a unified foundation model for promptable segmentation in images and videos. It can detect, segment, and track objects using text or visual prompts such as points, boxes, and masks.
---
...
**Source:** https://fal.ai/models/fal-ai/sam-3/image-rle/api
...
| --- | --- | --- |
| `rle` | string | list<string> | Run Length Encoding of the mask. |
...
| `metadata` | list<MaskMetadata> | null | Per-mask metadata when multiple RLEs are returned. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/sam-3/image-rle", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/sam-3/image-rle", {
...
```javascript
const result = await fal.queue.result("fal-ai/sam-3/image-rle", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/sam-3/image-rle/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/sam-3/image-rle
