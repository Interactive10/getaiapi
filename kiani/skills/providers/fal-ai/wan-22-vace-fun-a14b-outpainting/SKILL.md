---
name: fal-ai-wan-22-vace-fun-a14b-outpainting
description: >
  Use this skill for the fal.ai Wan 2.2 VACE Fun A14B model (fal-ai/wan-22-vace-fun-a14b/outpainting). VACE Fun for Wan 2.2 A14B from Alibaba-PAI
---
...
**Endpoint:** `fal-ai/wan-22-vace-fun-a14b/outpainting`
**Source:** https://fal.ai/models/fal-ai/wan-22-vace-fun-a14b/outpainting/api
...
| `video_url` | string | **Yes** |  | URL to the source video file. Required for outpainting. |
...
| `expand_ratio` | float | No | `0.25` | Amount of expansion. This is a float value between 0 and 1, where 0.25 adds 25% to the original video size on the spe... |
| `expand_bottom` | boolean | No | `false` | Whether to expand the video to the bottom. |
...
| `expand_left` | boolean | No | `false` | Whether to expand the video to the left. |
...
| `expand_top` | boolean | No | `false` | Whether to expand the video to the top. |
...
| `expand_right` | boolean | No | `false` | Whether to expand the video to the right. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-22-vace-fun-a14b/outpainting", {
  input: {
        "prompt": "your value here",
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-22-vace-fun-a14b/outpainting", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-22-vace-fun-a14b/outpainting", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-22-vace-fun-a14b/outpainting/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-22-vace-fun-a14b/outpainting
