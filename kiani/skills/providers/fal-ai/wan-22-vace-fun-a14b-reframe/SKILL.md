---
name: fal-ai-wan-22-vace-fun-a14b-reframe
description: >
  Use this skill for the fal.ai Wan 2.2 VACE Fun A14B model (fal-ai/wan-22-vace-fun-a14b/reframe). VACE Fun for Wan 2.2 A14B from Alibaba-PAI
---
...
**Endpoint:** `fal-ai/wan-22-vace-fun-a14b/reframe`
**Source:** https://fal.ai/models/fal-ai/wan-22-vace-fun-a14b/reframe/api
...
const result = await fal.subscribe("fal-ai/wan-22-vace-fun-a14b/reframe", {
  input: {
...
| `match_input_num_frames` | boolean | No | `true` | If true, the number of frames in the generated video will match the number of frames in the input video. If false, th... |
...
| `match_input_frames_per_second` | boolean | No | `true` | If true, the frames per second of the generated video will match the input video. If false, the frames per second wil... |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-22-vace-fun-a14b/reframe", {
  input: {
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-22-vace-fun-a14b/reframe", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-22-vace-fun-a14b/reframe", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-22-vace-fun-a14b/reframe/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-22-vace-fun-a14b/reframe
