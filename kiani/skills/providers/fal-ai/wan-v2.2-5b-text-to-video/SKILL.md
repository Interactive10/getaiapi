---
name: fal-ai-wan-v2.2-5b-text-to-video
description: >
  Use this skill for the fal.ai Wan v2.2 5B model (fal-ai/wan/v2.2-5b/text-to-video). Wan 2.2's 5B model produces up to 5 seconds of video 720p at 24FPS with fluid motion and powerful prompt understanding
---
...
**Source:** https://fal.ai/models/fal-ai/wan/v2.2-5b/text-to-video/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan/v2.2-5b/text-to-video", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan/v2.2-5b/text-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan/v2.2-5b/text-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan/v2.2-5b/text-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan/v2.2-5b/text-to-video
