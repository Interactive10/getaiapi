...
description: >
  Use this skill for the fal.ai Wan model (fal-ai/wan/v2.2-a14b/image-to-image). Wan 2.2's 14B model edit high-resolution, photorealistic images with powerful prompt understanding and fine-grained visual detail
---
...
**Source:** https://fal.ai/models/fal-ai/wan/v2.2-a14b/image-to-image/api
...
| `shift` | float | No | `2` |  |
| `image_size` | ImageSize | enum: `square_hd`, `square`, `portrait_4_3`, `portrait_16_9`, `landscape_4_3`, `landscape_16_9` | null | No |  |  |
...
| `aspect_ratio` | enum: `auto`, `16:9`, `9:16`, `1:1` | No | `"auto"` | Aspect ratio of the generated image. If 'auto', the aspect ratio will be determined automatically based on the input ... |
...
| `strength` | float | No | `0.5` | Denoising strength. 1.0 = fully remake; 0.0 = preserve original. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan/v2.2-a14b/image-to-image", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan/v2.2-a14b/image-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan/v2.2-a14b/image-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan/v2.2-a14b/image-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan/v2.2-a14b/image-to-image
