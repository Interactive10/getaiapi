---
name: fal-ai-wan-v2.2-a14b-image-to-video
description: >
  Use this skill for the fal.ai Wan v2.2 A14B model (fal-ai/wan/v2.2-a14b/image-to-video). fal-ai/wan/v2.2-A14B/image-to-video
---
...
**Source:** https://fal.ai/models/fal-ai/wan/v2.2-a14b/image-to-video/api
...
| `guidance_scale_2` | float | No | `3.5` | Guidance scale for the second stage of the model. This is used to control the adherence to the prompt in the second s... |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan/v2.2-a14b/image-to-video", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan/v2.2-a14b/image-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan/v2.2-a14b/image-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan/v2.2-a14b/image-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan/v2.2-a14b/image-to-video
