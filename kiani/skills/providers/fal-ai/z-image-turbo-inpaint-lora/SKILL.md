---
name: fal-ai-z-image-turbo-inpaint-lora
description: >
  Use this skill for the fal.ai Z-Image Turbo model (fal-ai/z-image/turbo/inpaint/lora). Generate images from text, an image, a mask and custom LoRA using Z-Image Turbo, Tongyi-MAI's super-fast 6B model.
---
...
**Source:** https://fal.ai/models/fal-ai/z-image/turbo/inpaint/lora/api
...
const result = await fal.subscribe("fal-ai/z-image/turbo/inpaint/lora", {
  input: {
        ...
        "mask_image_url": "https://example.com/input.png",
...
| `mask_image_url` | string | **Yes** |  | URL of Mask for Inpaint generation. |
...
| `image_url` | string | **Yes** |  | URL of Image for Inpaint generation. |
| `sync_mode` | boolean | No | `false` | If `True`, the media will be returned as a data URI and the output data won't be available in the request history. |
| `strength` | float | No | `1` | The strength of the inpaint conditioning. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/z-image/turbo/inpaint/lora", {
  input: {
        "prompt": "your value here",
        "mask_image_url": "https://example.com/input.png",
        "image_url": "https://example.com/input.png"
...
```javascript
const status = await fal.queue.status("fal-ai/z-image/turbo/inpaint/lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/z-image/turbo/inpaint/lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/z-image/turbo/inpaint/lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/z-image/turbo/inpaint/lora
