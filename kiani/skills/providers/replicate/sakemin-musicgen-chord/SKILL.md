---
name: replicate-sakemin-musicgen-chord
description: >
  Use this skill for the Replicate Musicgen Chord model (sakemin/musicgen-chord). Use the Musicgen Chord model via Replicate API.
---

# Musicgen Chord

**Model:** `sakemin/musicgen-chord`
**Source:** https://replicate.com/sakemin/musicgen-chord
...
const output = await replicate.run("sakemin/musicgen-chord", {
  input: {
...
output = replicate.run("sakemin/musicgen-chord",
    input={
        "prompt": "your prompt here"
    },
)
...
```bash
curl -s -X POST https://api.replicate.com/v1/models/sakemin/musicgen-chord/predictions \
...
| `time_sig` | string | No |  | Time signature value for the generate output. `text_chords` will be processed based on this value. This will be appen... |
...
| `multi_band_diffusion` | boolean | No | `false` | If `True`, the EnCodec tokens will be decoded with MultiBand Diffusion. |
...
- Model page: https://replicate.com/sakemin/musicgen-chord
- API page: https://replicate.com/sakemin/musicgen-chord/api
