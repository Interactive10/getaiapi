---
id: "033"
title: "`listModels()` now accepts `input`/`output` modality filters instead of `category`"
priority: P2
complexity: S
depends_on: []
area: "build"
...
# PDD-033: `listModels()` now accepts `input`/`output` modality filters instead of `category`
...
Completion of `listmodels()` now accepts `input`/`output` modality filters instead of `category`.
...
- AC-1: `listModels()` now accepts `input`/`output` modality filters instead of `category`

### Task breakdown
- [x] `listModels()` now accepts `input`/`output` modality filters instead of `category`
