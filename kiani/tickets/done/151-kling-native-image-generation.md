...
### Task breakdown
- [x] Read SKILL.md and map native Kling params for `v1/images/generations`
- [x] Add kling provider to kling-image-v3-text-to-image
- [x] Add kling provider to kling-image-v3-image-to-image
- [x] Verify O3 models use same endpoint or omni-image; add kling provider accordingly
- [x] Write unit test for param mapping
- [x] Run check:types
...
| Check | Passed | Notes |
|-------|--------|-------|
| Injection (SQL, NoSQL, command) | [x] | Registry JSON data only, no user input execution |
| Broken access control | [x] | Auth via KLING_ACCESS_KEY env var, no new auth paths |
| Sensitive data exposure | [x] | No secrets in registry |
| XSS | [x] | N/A — library code, no HTML output |
| Security misconfiguration | [x] | Follows existing provider pattern |
...
| AC | Verified | Notes |
|----|----------|-------|
| AC-1 | [x] | 4 image gen models have kling provider (v3 t2i, v3 i2i, o3 t2i, o3 i2i) |
| AC-2 | [x] | param_map verified against SKILL.md — no phantom params |
| AC-3 | [x] | extract_path task_result.images[].url matches Kling native response |
| AC-4 | [x] | tsc --noEmit passes |
| AC-5 | [x] | 9 unit tests pass (defaults, mapping, output, phantom exclusion) |
