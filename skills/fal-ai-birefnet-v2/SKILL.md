...
description: >
  Use this skill for the fal.ai Birefnet Background Removal model (fal-ai/birefnet/v2). bilateral reference framework (BiRefNet) for high-resolution dichotomous image segmentation (DIS)
---
...
**Source:** https://fal.ai/models/fal-ai/birefnet/v2/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/birefnet/v2", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/birefnet/v2", {
...
```javascript
const result = await fal.queue.result("fal-ai/birefnet/v2", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/birefnet/v2/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/birefnet/v2
