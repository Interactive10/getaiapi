---
name: fal-ai-bria-fibo-lite-generate
description: >
  Use this skill for the fal.ai Fibo Lite model (bria/fibo-lite/generate). Fibo Lite, the new addition to the Fibo model family, allows generating high-quality images with the same controllability of the JSON structured prompt with significantly improved latency.
---
...
**Endpoint:** `bria/fibo-lite/generate`
**Source:** https://fal.ai/models/bria/fibo-lite/generate/api
...
| `steps_num` | integer | No | `8` | Number of inference steps. |
...
```javascript
const { request_id } = await fal.queue.submit("bria/fibo-lite/generate", {
  input: {
        "prompt": "your prompt here"
      },
...
```javascript
const status = await fal.queue.status("bria/fibo-lite/generate", {
...
```javascript
const result = await fal.queue.result("bria/fibo-lite/generate", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/bria/fibo-lite/generate/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=bria/fibo-lite/generate
