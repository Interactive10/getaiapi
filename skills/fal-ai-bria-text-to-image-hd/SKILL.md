---
name: fal-ai-bria-text-to-image-hd
description: >
  Use this skill for the fal.ai Bria Text-to-Image HD model (fal-ai/bria/text-to-image/hd). Bria's Text-to-Image model for HD images. Trained exclusively on licensed data for safe and risk-free commercial use. Available also as source code and weights. For access to weights: https://bria.ai/
---
...
**Endpoint:** `fal-ai/bria/text-to-image/hd`
**Source:** https://fal.ai/models/fal-ai/bria/text-to-image/hd/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/bria/text-to-image/hd", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/bria/text-to-image/hd", {
...
```javascript
const result = await fal.queue.result("fal-ai/bria/text-to-image/hd", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/bria/text-to-image/hd/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/bria/text-to-image/hd
