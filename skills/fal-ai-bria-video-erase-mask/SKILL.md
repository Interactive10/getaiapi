---
name: fal-ai-bria-video-erase-mask
description: >
  Use this skill for the fal.ai Video model (bria/video/erase/mask). A high-fidelity capability for erasing unwanted objects, people, or visual elements from videos while maintaining aesthetic quality and temporal consistency.
---
...
**Source:** https://fal.ai/models/bria/video/erase/mask/api
...
```javascript
const { request_id } = await fal.queue.submit("bria/video/erase/mask", {
  input: {
        "video_url": "https://example.com/input.png",
        "mask_video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("bria/video/erase/mask", {
...
```javascript
const result = await fal.queue.result("bria/video/erase/mask", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/bria/video/erase/mask/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=bria/video/erase/mask
