---
name: fal-ai-cogvideox-5b-image-to-video
description: >
  Use this skill for the fal.ai CogVideoX-5B model (fal-ai/cogvideox-5b/image-to-video). Generate videos from images and prompts using CogVideoX-5B
---
...
**Source:** https://fal.ai/models/fal-ai/cogvideox-5b/image-to-video/api
...
| `image_url` | string | **Yes** |  | The URL to the image to generate the video from. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/cogvideox-5b/image-to-video", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/cogvideox-5b/image-to-video", {
...
```javascript
const result = await fal.queue.result("fal-ai/cogvideox-5b/image-to-video", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/cogvideox-5b/image-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/cogvideox-5b/image-to-video
