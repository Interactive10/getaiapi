...
name: fal-ai-editto
description: >
  Use this skill for the fal.ai Editto model (fal-ai/editto). Edit videos using instruction-based prompting using Editto model!
---
...
**Endpoint:** `fal-ai/editto`
**Source:** https://fal.ai/models/fal-ai/editto/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/editto", {
  input: {
        "prompt": "your value here",
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/editto", {
...
```javascript
const result = await fal.queue.result("fal-ai/editto", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/editto/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/editto
