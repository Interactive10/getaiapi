---
name: fal-ai-fast-svd-text-to-video
description: >
  Use this skill for the fal.ai Stable Video Diffusion model (fal-ai/fast-svd/text-to-video). Generate short video clips from your prompts using SVD v1.1
---
...
**Endpoint:** `fal-ai/fast-svd/text-to-video`
**Source:** https://fal.ai/models/fal-ai/fast-svd/text-to-video/api
...
| `deep_cache` | enum: `none`, `minimum`, `medium`, `high` | No | `"none"` | Enabling [DeepCache](https://github.com/horseee/DeepCache) will make the execution             faster, but might some... |
...
| `steps` | integer | No | `20` | The number of steps to run the model for. The higher the number the better             the quality and longer it will... |
...
| `negative_prompt` | string | No | `"unrealistic, saturated, high contrast, big nose, painting, drawing, sketch, cartoon, anime, manga, render, CG, 3d, watermark, signature, label"` | The negative prompt to use as a starting point for the generation. |
...
- API page: https://fal.ai/models/fal-ai/fast-svd/text-to-video/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/fast-svd/text-to-video
