---
name: fal-ai-flux-1-dev-redux
description: >
  Use this skill for the fal.ai FLUX.1 [dev] Redux model (fal-ai/flux-1/dev/redux). FLUX.1 [dev] Redux is a high-performance endpoint for the FLUX.1 [dev] model that enables rapid transformation of existing images, delivering high-quality style transfers and image modifications with 
---
...
**Endpoint:** `fal-ai/flux-1/dev/redux`
**Source:** https://fal.ai/models/fal-ai/flux-1/dev/redux/api
...
const result = await fal.subscribe("fal-ai/flux-1/dev/redux", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-1/dev/redux", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-1/dev/redux", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-1/dev/redux", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-1/dev/redux/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-1/dev/redux
