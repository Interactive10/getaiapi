...
description: >
  Use this skill for the fal.ai Flux 2 model (fal-ai/flux-2/edit). Image-to-image editing with FLUX.2 [dev] from Black Forest Labs. Precise modifications using natural language descriptions and hex color control.
---
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2/edit", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2/edit", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2/edit", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2/edit/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2/edit
