---
name: fal-ai-flux-2-lora-gallery-apartment-staging
description: >
  Use this skill for the fal.ai Flux 2 Lora Gallery model (fal-ai/flux-2-lora-gallery/apartment-staging). Virtually furnishes an empty apartment
---
...
**Endpoint:** `fal-ai/flux-2-lora-gallery/apartment-staging`
**Source:** https://fal.ai/models/fal-ai/flux-2-lora-gallery/apartment-staging/api
...
| --- | --- | --- | --- | --- |
| `prompt` | string | **Yes** |  | The prompt to generate a furnished room. Use 'furnish this room' for best results. |
...
| `lora_scale` | float | No | `1` | The strength of the apartment staging effect. |
...
| `image_urls` | list<string> | **Yes** |  | The URL of the empty room image to furnish. |
...
| `images` | list<Image> | The generated furnished room images |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2-lora-gallery/apartment-staging", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2-lora-gallery/apartment-staging", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2-lora-gallery/apartment-staging", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2-lora-gallery/apartment-staging/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2-lora-gallery/apartment-staging
