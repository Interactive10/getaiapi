---
name: fal-ai-flux-2-max-edit
description: >
  Use this skill for the fal.ai Flux 2 Max model (fal-ai/flux-2-max/edit). FLUX.2 [max] delivers state-of-the-art image generation and advanced image editing with exceptional realism, precision, and consistency.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-2-max/edit/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2-max/edit", {
  input: {
        "prompt": "your value here",
        "image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2-max/edit", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2-max/edit", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2-max/edit/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2-max/edit
