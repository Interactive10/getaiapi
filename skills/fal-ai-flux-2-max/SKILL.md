...
description: >
  Use this skill for the fal.ai Flux 2 Max model (fal-ai/flux-2-max). FLUX.2 [max] delivers state-of-the-art image generation and advanced image editing with exceptional realism, precision, and consistency.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-2-max/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-2-max", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-2-max", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-2-max", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-2-max/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-2-max
