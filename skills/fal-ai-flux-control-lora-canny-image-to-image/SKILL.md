---
name: fal-ai-flux-control-lora-canny-image-to-image
description: >
  Use this skill for the fal.ai FLUX.1 [dev] Control LoRA Canny model (fal-ai/flux-control-lora-canny/image-to-image). FLUX Control LoRA Canny is a high-performance endpoint that uses a control image using a Canny edge map to transfer structure to the generated image and another initial image to guide color.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-control-lora-canny/image-to-image/api
...
const result = await fal.subscribe("fal-ai/flux-control-lora-canny/image-to-image", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-control-lora-canny/image-to-image", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png",
        "control_lora_image_url": "https://example.com/input.png"
...
```javascript
const status = await fal.queue.status("fal-ai/flux-control-lora-canny/image-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-control-lora-canny/image-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-control-lora-canny/image-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-control-lora-canny/image-to-image
