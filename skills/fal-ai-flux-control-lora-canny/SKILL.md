---
name: fal-ai-flux-control-lora-canny
description: >
  Use this skill for the fal.ai FLUX.1 [dev] Control LoRA Canny model (fal-ai/flux-control-lora-canny). FLUX Control LoRA Canny is a high-performance endpoint that uses a control image to transfer structure to the generated image, using a Canny edge map.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-control-lora-canny/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-control-lora-canny", {
  input: {
        "prompt": "your value here",
        "control_lora_image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-control-lora-canny", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-control-lora-canny", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-control-lora-canny/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-control-lora-canny
