---
name: fal-ai-flux-krea-lora
description: >
  Use this skill for the fal.ai FLUX.1 Krea [dev] with LoRAs model (fal-ai/flux-krea-lora). Super fast endpoint for the FLUX.1 [dev] model with LoRA support, enabling rapid and high-quality image generation using pre-trained LoRA adaptations for personalization, specific styles, brand identi
---
...
**Source:** https://fal.ai/models/fal-ai/flux-krea-lora/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-krea-lora", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-krea-lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-krea-lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-krea-lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-krea-lora
