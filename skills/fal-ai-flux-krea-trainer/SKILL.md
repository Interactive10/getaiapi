---
name: fal-ai-flux-krea-trainer
description: >
  Use this skill for the fal.ai Train Flux Krea LoRA model (fal-ai/flux-krea-trainer). Train styles, people and other subjects at blazing speeds using the FLUX.1 Krea [dev] base model.
---
...
**Source:** https://fal.ai/models/fal-ai/flux-krea-trainer/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-krea-trainer", {
  input: {
        "images_data_url": "https://example.com/input.png"
    },
...
```javascript
const result = await fal.queue.result("fal-ai/flux-krea-trainer", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-krea-trainer/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-krea-trainer
