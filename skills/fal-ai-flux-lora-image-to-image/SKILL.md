---
name: fal-ai-flux-lora-image-to-image
description: >
  Use this skill for the fal.ai FLUX.1 [dev] with LoRAs model (fal-ai/flux-lora/image-to-image). FLUX LoRA Image-to-Image is a high-performance endpoint that transforms existing images using FLUX models, leveraging LoRA adaptations to enable rapid and precise image style transfer, modifications, 
---
...
**Source:** https://fal.ai/models/fal-ai/flux-lora/image-to-image/api
...
```javascript
const status = await fal.queue.status("fal-ai/flux-lora/image-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-lora/image-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-lora/image-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-lora/image-to-image
