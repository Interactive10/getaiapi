---
name: fal-ai-flux-schnell-redux
description: >
  Use this skill for the fal.ai FLUX.1 [schnell] Redux model (fal-ai/flux/schnell/redux). FLUX.1 [schnell] Redux is a high-performance endpoint for the FLUX.1 [schnell] model that enables rapid transformation of existing images, delivering high-quality style transfers and image modificatio
---
...
**Endpoint:** `fal-ai/flux/schnell/redux`
**Source:** https://fal.ai/models/fal-ai/flux/schnell/redux/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux/schnell/redux", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux/schnell/redux", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux/schnell/redux", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux/schnell/redux/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux/schnell/redux
