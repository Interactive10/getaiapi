...
description: >
  Use this skill for the fal.ai FLUX.1 Subject model (fal-ai/flux-subject). Super fast endpoint for the FLUX.1 [schnell] model with subject input capabilities, enabling rapid and high-quality image generation for personalization, specific styles, brand identities, and product
---
...
**Source:** https://fal.ai/models/fal-ai/flux-subject/api
...
| `image_url` | string | **Yes** |  | URL of image of the subject |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/flux-subject", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/flux-subject", {
...
```javascript
const result = await fal.queue.result("fal-ai/flux-subject", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/flux-subject/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/flux-subject
