---
name: fal-ai-glm-image
description: >
  Use this skill for the fal.ai Glm Image model (fal-ai/glm-image). Create high-quality images with accurate text rendering and rich knowledge details—supports editing, style transfer, and maintaining consistent characters across multiple images.
---
...
**Endpoint:** `fal-ai/glm-image`
**Source:** https://fal.ai/models/fal-ai/glm-image/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/glm-image", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/glm-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/glm-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/glm-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/glm-image
