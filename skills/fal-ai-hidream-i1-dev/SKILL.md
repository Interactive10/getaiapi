...
description: >
  Use this skill for the fal.ai Hidream I1 Dev model (fal-ai/hidream-i1-dev). HiDream-I1 dev is a new open-source image generative foundation model with 17B parameters that achieves state-of-the-art image generation quality within seconds.
---
...
**Source:** https://fal.ai/models/fal-ai/hidream-i1-dev/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/hidream-i1-dev", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/hidream-i1-dev", {
...
```javascript
const result = await fal.queue.result("fal-ai/hidream-i1-dev", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/hidream-i1-dev/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/hidream-i1-dev
