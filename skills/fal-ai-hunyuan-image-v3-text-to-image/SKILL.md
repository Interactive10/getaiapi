...
description: >
  Use this skill for the fal.ai Hunyuan Image model (fal-ai/hunyuan-image/v3/text-to-image). Leverage the state-of-the-art capabilities of Hunyuan Image 3.0 to generate visual content that effectively conveys the messaging of your written material.
---
...
**Source:** https://fal.ai/models/fal-ai/hunyuan-image/v3/text-to-image/api
...
| `guidance_scale` | float | No | `7.5` | Controls how much the model adheres to the prompt. Higher values mean stricter adherence. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/hunyuan-image/v3/text-to-image", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/hunyuan-image/v3/text-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/hunyuan-image/v3/text-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/hunyuan-image/v3/text-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/hunyuan-image/v3/text-to-image
