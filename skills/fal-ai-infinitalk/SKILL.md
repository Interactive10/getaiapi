...
description: >
  Use this skill for the fal.ai Infinitalk model (fal-ai/infinitalk). Infinitalk model generates a talking avatar video from an image and audio file. The avatar lip-syncs to the provided audio with natural facial expressions.
---
...
**Source:** https://fal.ai/models/fal-ai/infinitalk/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/infinitalk", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png",
        "audio_url": "https://example.com/input.png"
...
```javascript
const status = await fal.queue.status("fal-ai/infinitalk", {
...
```javascript
const result = await fal.queue.result("fal-ai/infinitalk", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/infinitalk/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/infinitalk
