---
name: fal-ai-lora-inpaint
description: >
  Use this skill for the fal.ai Stable Diffusion with LoRAs model (fal-ai/lora/inpaint). Run Any Stable Diffusion model with customizable LoRA weights.
---
...
**Endpoint:** `fal-ai/lora/inpaint`
**Source:** https://fal.ai/models/fal-ai/lora/inpaint/api
...
const result = await fal.subscribe("fal-ai/lora/inpaint", {
  input: {
...
| `noise_strength` | float | No | `0.5` | The amount of noise to add to noise image for image. Only used if the image_url is provided. 1.0 is complete noise an... |
...
| `image_url` | string | No |  | URL of image to use for image to image/inpainting. |
...
| `mask_url` | string | No |  | URL of black-and-white image to use as mask during inpainting. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/lora/inpaint", {
  input: {
        "prompt": "your value here",
        "model_name": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/lora/inpaint", {
...
```javascript
const result = await fal.queue.result("fal-ai/lora/inpaint", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/lora/inpaint/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/lora/inpaint
