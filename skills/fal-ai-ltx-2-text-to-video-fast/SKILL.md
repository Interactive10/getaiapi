...
description: >
  Use this skill for the fal.ai LTX Video 2.0 Fast model (fal-ai/ltx-2/text-to-video/fast). Create high-fidelity video with audio from text with LTX-2 Fast
---
...
**Source:** https://fal.ai/models/fal-ai/ltx-2/text-to-video/fast/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ltx-2/text-to-video/fast", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ltx-2/text-to-video/fast", {
...
```javascript
const result = await fal.queue.result("fal-ai/ltx-2/text-to-video/fast", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ltx-2/text-to-video/fast/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2/text-to-video/fast
