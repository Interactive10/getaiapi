...
name: fal-ai-magi
description: >
  Use this skill for the fal.ai MAGI-1 model (fal-ai/magi). MAGI-1 is a video generation model with exceptional understanding of physical interactions and cinematic prompts
---
...
**Endpoint:** `fal-ai/magi`
**Source:** https://fal.ai/models/fal-ai/magi/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/magi", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/magi", {
...
```javascript
const result = await fal.queue.result("fal-ai/magi", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/magi/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/magi
