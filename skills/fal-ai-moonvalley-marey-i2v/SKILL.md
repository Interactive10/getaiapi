---
name: fal-ai-moonvalley-marey-i2v
description: >
  Use this skill for the fal.ai Marey Realism V1.5 model (moonvalley/marey/i2v). Generate a video starting from an image as the first frame with Marey, a generative video model trained exclusively on fully licensed data.
---
...
**Endpoint:** `moonvalley/marey/i2v`
**Source:** https://fal.ai/models/moonvalley/marey/i2v/api
...
| `dimensions` | enum: `1920x1080`, `1080x1920`, `1152x1152`, `1536x1152`, `1152x1536` | No | `"1920x1080"` | The dimensions of the generated video in width x height format. |
...
```javascript
const { request_id } = await fal.queue.submit("moonvalley/marey/i2v", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("moonvalley/marey/i2v", {
...
```javascript
const result = await fal.queue.result("moonvalley/marey/i2v", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/moonvalley/marey/i2v/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=moonvalley/marey/i2v
