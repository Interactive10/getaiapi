---
name: fal-ai-one-to-all-animation-14b
description: >
  Use this skill for the fal.ai One To All Animation model (fal-ai/one-to-all-animation/14b). One-to-All Animation is a pose driven video model that animates characters from a single reference image, enabling flexible, alignment-free motion transfer across diverse styles and scenes
---
...
**Endpoint:** `fal-ai/one-to-all-animation/14b`
**Source:** https://fal.ai/models/fal-ai/one-to-all-animation/14b/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/one-to-all-animation/14b", {
  input: {
        "prompt": "your value here",
        "video_url": "https://example.com/input.png",
        "image_url": "https://example.com/input.png",
...
```javascript
const status = await fal.queue.status("fal-ai/one-to-all-animation/14b", {
...
```javascript
const result = await fal.queue.result("fal-ai/one-to-all-animation/14b", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/one-to-all-animation/14b/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/one-to-all-animation/14b
