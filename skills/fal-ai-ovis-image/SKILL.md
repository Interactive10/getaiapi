---
name: fal-ai-ovis-image
description: >
  Use this skill for the fal.ai Ovis Image model (fal-ai/ovis-image). Ovis-Image is a 7B text-to-image model specifically optimized for quick, high quality text rendering.
---
...
**Endpoint:** `fal-ai/ovis-image`
**Source:** https://fal.ai/models/fal-ai/ovis-image/api
...
const result = await fal.subscribe("fal-ai/ovis-image", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/ovis-image", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/ovis-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/ovis-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/ovis-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ovis-image
