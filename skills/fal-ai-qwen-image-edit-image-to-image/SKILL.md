---
name: fal-ai-qwen-image-edit-image-to-image
description: >
  Use this skill for the fal.ai Qwen Image Edit model (fal-ai/qwen-image-edit/image-to-image). Image to Image Endpoint for Qwen's Image Editing model. Has superior text editing capabilities.
---
...
**Source:** https://fal.ai/models/fal-ai/qwen-image-edit/image-to-image/api
...
const result = await fal.subscribe("fal-ai/qwen-image-edit/image-to-image", {
  input: {
...
| `strength` | float | No | `0.94` | Strength of the image-to-image transformation. Lower values preserve more of the original image. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/qwen-image-edit/image-to-image", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/qwen-image-edit/image-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/qwen-image-edit/image-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/qwen-image-edit/image-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/qwen-image-edit/image-to-image
