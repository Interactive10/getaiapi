...
description: >
  Use this skill for the fal.ai Qwen Image Layered model (fal-ai/qwen-image-layered). Qwen-Image-Layered is a model capable of decomposing an image into multiple RGBA layers.
---
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/qwen-image-layered", {
  input: {
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/qwen-image-layered", {
...
```javascript
const result = await fal.queue.result("fal-ai/qwen-image-layered", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/qwen-image-layered/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/qwen-image-layered
