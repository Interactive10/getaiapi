...
description: >
  Use this skill for the fal.ai Recraft V4 model (fal-ai/recraft/v4/text-to-image). Recraft V4 was developed with designers to bring true visual taste to AI image generation. Built for brand systems and production-ready workflows, it goes beyond prompt accuracy delivering stronger co
---
...
**Source:** https://fal.ai/models/fal-ai/recraft/v4/text-to-image/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/recraft/v4/text-to-image", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/recraft/v4/text-to-image", {
...
```javascript
const result = await fal.queue.result("fal-ai/recraft/v4/text-to-image", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/recraft/v4/text-to-image/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/recraft/v4/text-to-image
