---
name: fal-ai-rundiffusion-fal-juggernaut-flux-lora
description: >
  Use this skill for the fal.ai Juggernaut Flux Base LoRA model (rundiffusion-fal/juggernaut-flux-lora). Juggernaut Base Flux LoRA by RunDiffusion is a drop-in replacement for Flux [Dev] that delivers sharper details, richer colors, and enhanced realism to all your LoRAs and LyCORIS with full compatibili
---
...
**Source:** https://fal.ai/models/rundiffusion-fal/juggernaut-flux-lora/api
...
const result = await fal.subscribe("rundiffusion-fal/juggernaut-flux-lora", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("rundiffusion-fal/juggernaut-flux-lora", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("rundiffusion-fal/juggernaut-flux-lora", {
...
```javascript
const result = await fal.queue.result("rundiffusion-fal/juggernaut-flux-lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/rundiffusion-fal/juggernaut-flux-lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=rundiffusion-fal/juggernaut-flux-lora
