---
name: fal-ai-stable-cascade
description: >
  Use this skill for the fal.ai Stable Cascade model (fal-ai/stable-cascade). Stable Cascade: Image generation on a smaller & cheaper latent space.
---
...
**Source:** https://fal.ai/models/fal-ai/stable-cascade/api
...
| `second_stage_guidance_scale` | float | No | `0` | The CFG (Classifier Free Guidance) scale is a measure of how close you want             the model to stick to your pr... |
| `sync_mode` | boolean | No | `false` | If set to true, the image will be returned as base64 encoded string. |
| `first_stage_steps` | integer | No | `20` | Number of steps to run the first stage for. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/stable-cascade", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/stable-cascade", {
...
```javascript
const result = await fal.queue.result("fal-ai/stable-cascade", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/stable-cascade/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/stable-cascade
