...
name: fal-ai-uno
description: >
  Use this skill for the fal.ai Uno model (fal-ai/uno). An AI model that transforms input images into new ones based on text prompts, blending reference visuals with your creative directions.
---
...
**Endpoint:** `fal-ai/uno`
**Source:** https://fal.ai/models/fal-ai/uno/api
...
const result = await fal.subscribe("fal-ai/uno", {
  input: {
...
| `input_image_urls` | list<string> | **Yes** |  | URL of images to use while generating the image. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/uno", {
  input: {
        "prompt": "your value here",
        "input_image_urls": []
    },
...
```javascript
const status = await fal.queue.status("fal-ai/uno", {
...
```javascript
const result = await fal.queue.result("fal-ai/uno", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/uno/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/uno
