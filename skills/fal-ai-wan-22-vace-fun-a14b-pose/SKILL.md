---
name: fal-ai-wan-22-vace-fun-a14b-pose
description: >
  Use this skill for the fal.ai Wan 2.2 VACE Fun A14B model (fal-ai/wan-22-vace-fun-a14b/pose). VACE Fun for Wan 2.2 A14B from Alibaba-PAI
---
...
**Endpoint:** `fal-ai/wan-22-vace-fun-a14b/pose`
**Source:** https://fal.ai/models/fal-ai/wan-22-vace-fun-a14b/pose/api
...
const result = await fal.subscribe("fal-ai/wan-22-vace-fun-a14b/pose", {
  input: {
...
| --- | --- | --- | --- | --- |
| `prompt` | string | **Yes** |  | The text prompt to guide video generation. For pose task, the prompt should describe the desired pose and action of t... |
| `video_url` | string | **Yes** |  | URL to the source video file. Required for pose task. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-22-vace-fun-a14b/pose", {
  input: {
        "prompt": "your value here",
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-22-vace-fun-a14b/pose", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-22-vace-fun-a14b/pose", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-22-vace-fun-a14b/pose/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-22-vace-fun-a14b/pose
