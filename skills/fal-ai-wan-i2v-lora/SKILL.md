---
name: fal-ai-wan-i2v-lora
description: >
  Use this skill for the fal.ai Wan-2.1 Image-to-Video with LoRAs model (fal-ai/wan-i2v-lora). Add custom LoRAs to Wan-2.1 is a image-to-video model that generates high-quality videos with high visual quality and motion diversity from images
---
...
**Source:** https://fal.ai/models/fal-ai/wan-i2v-lora/api
...
| `aspect_ratio` | enum: `auto`, `16:9`, `9:16`, `1:1` | No | `"16:9"` | Aspect ratio of the output video. |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-i2v-lora", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-i2v-lora", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-i2v-lora", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-i2v-lora/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-i2v-lora
