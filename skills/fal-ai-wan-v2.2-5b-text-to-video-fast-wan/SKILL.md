---
name: fal-ai-wan-v2.2-5b-text-to-video-fast-wan
description: >
  Use this skill for the fal.ai Wan model (fal-ai/wan/v2.2-5b/text-to-video/fast-wan). Wan 2.2's 5B FastVideo model produces up to 5 seconds of video 720p at 24FPS with fluid motion and powerful prompt understanding
---
...
**Source:** https://fal.ai/models/fal-ai/wan/v2.2-5b/text-to-video/fast-wan/api
...
| `resolution` | enum: `480p`, `580p`, `720p` | No | `"720p"` | Resolution of the generated video (580p or 720p). |
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan/v2.2-5b/text-to-video/fast-wan", {
  input: {
        "prompt": "your value here"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan/v2.2-5b/text-to-video/fast-wan", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan/v2.2-5b/text-to-video/fast-wan", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan/v2.2-5b/text-to-video/fast-wan/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan/v2.2-5b/text-to-video/fast-wan
