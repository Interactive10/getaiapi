---
name: fal-ai-wan-vace-14b-pose
description: >
  Use this skill for the fal.ai Wan VACE 14B model (fal-ai/wan-vace-14b/pose). VACE is a video generation model that uses a source image, mask, and video to create prompted videos with controllable sources.
---
...
**Source:** https://fal.ai/models/fal-ai/wan-vace-14b/pose/api
...
const result = await fal.subscribe("fal-ai/wan-vace-14b/pose", {
  input: {
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/wan-vace-14b/pose", {
  input: {
        "prompt": "your value here",
        "video_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/wan-vace-14b/pose", {
...
```javascript
const result = await fal.queue.result("fal-ai/wan-vace-14b/pose", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/wan-vace-14b/pose/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/wan-vace-14b/pose
