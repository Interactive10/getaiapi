---
name: fal-ai-z-image-turbo-controlnet
description: >
  Use this skill for the fal.ai Z-Image Turbo model (fal-ai/z-image/turbo/controlnet). Generate images from text and edge, depth or pose images using Z-Image Turbo, Tongyi-MAI's super-fast 6B model.
---
...
**Source:** https://fal.ai/models/fal-ai/z-image/turbo/controlnet/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/z-image/turbo/controlnet", {
  input: {
        "prompt": "your value here",
        "image_url": "https://example.com/input.png"
    },
...
```javascript
const status = await fal.queue.status("fal-ai/z-image/turbo/controlnet", {
...
```javascript
const result = await fal.queue.result("fal-ai/z-image/turbo/controlnet", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/z-image/turbo/controlnet/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/z-image/turbo/controlnet
