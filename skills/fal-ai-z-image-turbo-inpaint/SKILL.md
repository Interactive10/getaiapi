---
name: fal-ai-z-image-turbo-inpaint
description: >
  Use this skill for the fal.ai Z-Image Turbo model (fal-ai/z-image/turbo/inpaint). Generate images from text, an image and a mask using Z-Image Turbo, Tongyi-MAI's super-fast 6B model.
---
...
**Source:** https://fal.ai/models/fal-ai/z-image/turbo/inpaint/api
...
```javascript
const { request_id } = await fal.queue.submit("fal-ai/z-image/turbo/inpaint", {
  input: {
        "prompt": "your value here",
        "mask_image_url": "https://example.com/input.png",
        "image_url": "https://example.com/input.png"
...
```javascript
const status = await fal.queue.status("fal-ai/z-image/turbo/inpaint", {
...
```javascript
const result = await fal.queue.result("fal-ai/z-image/turbo/inpaint", {
  requestId: "<request_id>",
});
console.log(result.data);
```
...
- API page: https://fal.ai/models/fal-ai/z-image/turbo/inpaint/api
- OpenAPI spec: https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/z-image/turbo/inpaint
