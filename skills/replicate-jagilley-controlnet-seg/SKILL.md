---
name: replicate-jagilley-controlnet-seg
description: >
  Use this skill for the Replicate Controlnet Seg model (jagilley/controlnet-seg). Use the Controlnet Seg model via Replicate API.
---

# Controlnet Seg

**Model:** `jagilley/controlnet-seg`
**Source:** https://replicate.com/jagilley/controlnet-seg
...
const output = await replicate.run("jagilley/controlnet-seg", {
  input: {
...
output = replicate.run("jagilley/controlnet-seg",
    input={
        "image": "https://example.com/input.png",
        "prompt": "your prompt here"
    },
...
```bash
curl -s -X POST https://api.replicate.com/v1/models/jagilley/controlnet-seg/predictions \
...
| `detect_resolution` | integer | No | `512` | Resolution for detection (only applicable when model type is 'HED' or 'Segmentation') |
...
- Model page: https://replicate.com/jagilley/controlnet-seg
- API page: https://replicate.com/jagilley/controlnet-seg/api
